...
namespace NLogicLib
{
	...
	ERROR_CODE Main::Init(RankAndRdbmsUpdate * p_Update)
	{
		...
		//Mysql메니저 초기화
		m_pMysqlMgr = std::make_unique<MysqlManager>();
		m_pMysqlMgr->Init();
		...
		//서버가 시작되면 로그인 정보를 불러온다.
		m_pUserMgr->Init(m_pServerConfig->MaxClientCount, m_pMysqlMgr.get());
		...
		m_pPacketProc->Init(m_pNetwork.get(), m_pUserMgr.get(), m_pLobbyMgr.get(), m_pServerConfig.get(), m_pLogger.get(), m_pRedisMgr.get(),
			m_pMysqlMgr.get());
		...
		p_Update->Init(m_pRedisMgr.get(), m_pMysqlMgr.get(), m_pUserMgr.get(), m_pNetwork.get());
