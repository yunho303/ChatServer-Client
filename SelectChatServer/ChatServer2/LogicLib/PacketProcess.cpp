...
{	
	...
	void PacketProcess::Init(TcpNet* pNetwork, UserManager* pUserMgr, LobbyManager* pLobbyMgr, ServerConfig* pConfig, ILog* pLogger, RedisManager* pRedis,
		MysqlManager* pMysqlMgr)
	{
	...
	ERROR_CODE PacketProcess::NtfSysConnctSession(PacketInfo packetInfo)
	{
		...
		cout << "User" << packetInfo.SessionIndex<< " Connected"<<endl;
	...
	{		
		...
		cout << "Echo패킷처리" <<" "<< packetInfo.PacketBodySize<< endl;
		NCommon::PktDevEchoRes resPkt;
		resPkt.ErrorCode = (short)ERROR_CODE::NONE;
		//resPkt.DataSize = reqPkt->DataSize;
		std::memcpy(&resPkt.Datas, reqPkt->Datas, packetInfo.PacketBodySize);
		for (int i = 0; i < packetInfo.PacketBodySize; i++) {
			cout <<packetInfo.pRefData[i];
			//cout << resPkt.Datas[i];
		}
		cout << endl;
		auto sendSize = packetInfo.PacketBodySize+ sizeof(resPkt.ErrorCode);
